      ...
      _roomSubscription = _dbRef.child("rooms").child(widget.roomCode).onValue.listen((event) {
        ...
        if (roomPlayers.isNotEmpty) {
          ...
          if (firebaseDealtStatus && roomData['hands'] != null) {
            ...
            int maxTricksInDeck = (widget.totalPlayers == 2) ? 10 : (widget.totalPlayers == 3 ? 6 : 5);
            bool allHandsEmpty = game.players.isNotEmpty && game.players.every((p) => p.hand.isEmpty);
            bool isDeckOver = (game.currentTrickInDeck >= maxTricksInDeck) || allHandsEmpty;

            if (isBajiFinishedInFb || (isDeckOver && tableCards.isEmpty)) {
  ...
  void _handleCardTap(int cardValue) async {
    ...
    if (isLastCardOfTrick) {
      ...
      if (widget.mode == GameMode.friend && widget.roomCode.isNotEmpty) {
        ...
        int maxTricksInDeck = (widget.totalPlayers == 2) ? 10 : (widget.totalPlayers == 3 ? 6 : 5);
        bool isDeckOver = (game.currentTrickInDeck >= maxTricksInDeck) || game.players.every((p) => p.hand.isEmpty);
        bool isTargetHit = game.players.any((p) => p.currentScore >= game.targetScore);
        ...
        await _dbRef.child("rooms").child(widget.roomCode).update({
          ...
          "isBajiFinished": (isDeckOver && !isTargetHit),
          "cardsDealt": !isDeckOver, // Deck khatam hote hi cardsDealt false ho jayega
        });
      }
    }
    ...
    if (mounted) {
      ...
      if (widget.mode == GameMode.friend) {
        ...
        if (isDeckOver && game.currentRoundCards.isEmpty) {
          bool isTargetHit = game.players.any((p) => p.currentScore >= game.targetScore);
          if (!isTargetHit) {
            setState(() {
              game.isDeckFinished = true;
